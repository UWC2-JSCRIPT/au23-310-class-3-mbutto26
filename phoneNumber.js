...
function parsePhoneNumber(phoneNumbers) {
  const phoneNumberParts = /^\(?(\d{3})[\)\s-]+(\d{3})[\s-](\d{4})$/.exec(
    phoneNumbers
  );

  const [, areaCode, first3, last4] = phoneNumberParts;
  const phoneNumberObj = {
    areaCode,
    phoneNumbers: `${first3}${last4}`,
  };
